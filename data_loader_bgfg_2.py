...
class cityscape(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(num_object):
            ...
            bbox = instance['bbox']
            bbox_mask = Image.new('L', (img_size_x, img_size_y))
            ImageDraw.Draw(bbox_mask).rectangle([bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]], outline=1,
                                                fill=1)
            bbox_mask = self.transform2(bbox_mask)
            if torch.max(bbox_mask) != 0:
                bbox_mask = bbox_mask / torch.max(bbox_mask)
        ...
        sample = {'image': img, 'seg_mask': seg_masks, 'bboxes': bboxes, 'fg_category': fg_category,
                  'num_object': num_object, 'ins_area': ins_area}
        return sample
    ...
    def discard_small(self, min_area, max_area=1):
        ...
        for img_id in self.ids:
            ...
            if valid_mask and total_fg_area_relevant / (img_size_x * img_size_y) > min_area and total_fg_area / (
                    img_size_x * img_size_y) < max_area:
                temp.append(img_id)
    ...
    def discard_num_objects(self, num_min_obj=0, num_max_obj=1):
        ...
        for img_id in self.ids:
            ...
            idx_list = [i for i in range(len(instance_types)) if
                        (instance_types[i] in self.category and len(target[i]['segmentation']) == 1)]
            num_object = len(idx_list)
