...
from data_loader_fg_model import chairs
...
def main():
    ...
    parser.add_argument('--train_epoch', type=int, default=20,help='Maximum training epoch')
    ...
    allmasks = sorted(glob.glob(os.path.join(opt.mask_imgs, '**', '*.png'), recursive=True))
    print('Number of masks: %d' % len(allmasks))
    dataset = chairs(imfile= opt.train_imgs, mfiles = allmasks,  category_names = category_names, transform=transform, final_img_size=opt.img_size)
